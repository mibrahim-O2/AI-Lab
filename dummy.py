...
# Lab Guide [C]: small reusable "card" widget used throughout the
# result panels (Tab 1 outputs, Tab 4 metric comparison, etc.).
...
# Lab Guide [E]: grouped bar chart used to compare the Rule-Based
# Engine against the Decision Tree across all four metrics at once.
...
# Lab Guide [E]: heatmap of the confusion matrix -- this matters
# because accuracy alone hides WHICH topics the model confuses with
# one another, while this chart shows it directly.
        ...
        if run_btn:
            ...
            with st.spinner("Generating recommendation..."):
                ...
                if mode == "Rule-Based Engine":
                    ...
                    # This path is 100% hand-crafted logic -- transparent
                    # by design, with no training data required.
                    result = run_rules(
                        score         = score,
                        confidence    = confidence,
                        response_time = response_time,
                    ...
                    # This path is data-driven instead of hand-written,
                    # which is why Tab 3 has to reconstruct its reasoning
                    # afterwards (get_tree_explanation) rather than just
                    # printing a rule that was already known in advance.
                    result = run_model(input_data)

            # Cache the result in session_state because Streamlit reruns
            # this entire script top-to-bottom on every interaction --
            # without caching, switching tabs would lose the result or
            # re-trigger the engine unnecessarily.
